...
function App() {
  const routes = ['my-app','About','LogIn',  'Products', 'Cart'];
  return (
    <RecoilRoot>
            ...
            <nav id='navApp' className="navbar navbar-expand-lg bg-light container-fluid">
              <menu id='mainMenu'>
                <ul id='ulApp' className="navbar-nav me-auto mb-2 mb-lg-0">
                  {routes.map(route => <li id='liApp' className="dropdown-item"/*"nav-link active nav-item"*/ aria-current="page" key={route}><Link to={route}>{route}</Link></li>)}
                </ul>
              </menu>
            </nav>
          </div>
